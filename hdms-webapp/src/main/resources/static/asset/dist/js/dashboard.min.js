...
"use strict";
var dashboard = angular.module("dashboard", ["ngAnimate", "ngResource", "ui.router", "ui.bootstrap", "ui.validate", "pascalprecht.translate", "messager", "toaster", "hc.ui", "hc.util", "ui.jq"]);
dashboard.config(["$controllerProvider", "$compileProvider", "$filterProvider", "$provide", function (t, n, e, r) {
    dashboard.controller = t.register,
        dashboard.filter = e.register,
        dashboard.factory = r.factory,
        dashboard.service = r.service,
        dashboard.constant = r.constant,
        dashboard.value = r.value
}]),
window.require &&
function (t) {
    ...
    }), define(["dashboard/dashboard.config", "dashboard/controllers/dashboard", "dashboard/controllers/index", "dashboard/controllers/security", "dashboard/controllers/system", "dashboard/controllers/department", "dashboard/controllers/user"], t)) : t()
}(function () {}),
    dashboard.config(["$translateProvider", function (t) {
        t.useStaticFilesLoader({
            prefix: "/asset/l10n/",
            suffix: ".json"
        }),
            t.preferredLanguage("zh_CN")
    }]).config(["$httpProvider", function (t) {
        var n = {
            "Content-Type": "application/json",
            "X-Requested-With": "XMLHttpRequest"
        };
        n[crsf.headerName] = crsf.token,
            t.defaults.headers.post = n,
            t.defaults.headers.get = {
                "X-Requested-With": "XMLHttpRequest"
            }
    }]),
    dashboard.run(["$rootScope", "$state", "$stateParams", function (t, n, e) {
        t.$state = n,
            t.$stateParams = e
    }]).config(["$stateProvider", "$urlRouterProvider", function (t, n) {
        n.otherwise("/index"),
            t.state("/index", {
                url: "/index",
    ...
    angular.module("lc.atwho", []).directive("lcAtwho", ["$parse", "$filter", "emojis", "$rootScope", "bus", "$translate", "lcData", "LinkModuleService", "$http", "$q", function (t, n, e, r, s, a, i, l, o, c) {
        return {
            require: "^ngModel",
            scope: !0,
            link: function (i, o, c) {
                var d = t(c.atwhoKeydown),
                    p = {
                        emojis: !0,
                        member: !0,
                        channel: !0,
                ...
                c.lcAtwho && (p = $.extend(p, i.$eval(c.lcAtwho))),
                p.emojis && o.atwho({
                    at: ":",
                    data: e,
                    show_the_at: !0,
                ...
                p.entity && l.bindAtWho(o, c.channels && p.channel),
                c.atwhoMembers && p.member && i.$watch(c.atwhoMembers, function (t, e) {
                    var i = _.reject(t, function (t) {
                        return t.uid === r.global.me.uid
                    });
                    if (i = s.member.filterBot(i, !0), _.isEmpty(i)) o.atwho({
                        at: "@",
                        data: []
                    });
                    else {
                        _.forEach(i, function (t) {
                            0 === t.state ? t.lc_state = '<span class="fa fa-circle lc-state offline"></span>' : 1 === t.state ? t.lc_state = '<span class="fa fa-circle lc-state online"></span>' : 2 === t.state ? t.lc_state = '<span class="fa fa-minus-circle lc-state busy"></span>' : 3 === t.state && (t.lc_state = '<span class="fa fa-dot-circle-o lc-state away"></span>')
                        }),
                        c.hasGroup && i.push({
                            name: "group",
                            display_name: a.instant("atwho.GROUP"),
                            display_name_pinyin: "all"
                        });
                        var l = ["<li data-value='@${name} '>", "<div title='${display_name}' class='lc-avatar lc-avatar-30'>", "<span class='lc-avatar-def m-l-10 m-r-5' style='background-color:${bgColor};'><div>${shortName}</div></span></div>&nbsp;", "${display_name}", " <small>(${name})</small>", "</li>"].join(""),
                            d = '<li data-value="@${display_name} " title="@${display_name}" class="channel">@${display_name} <small>(' + a.instant("atwho.NOTICE_ALL") + ")</small></li>";
                        o.atwho({
                            at: "@",
                            limit: 1e4,
                            header_tpl: '<div class="header">' + a.instant("atwho.MEMBER") + "<small>↑&nbsp;↓&nbsp;" + a.instant("atwho.NAVIGATION") + "</small></div>",
                            data: i,
                            ...
                            callbacks: {
                                filter: function (t, n, e) {
                                    if (_.isEmpty(t)) return n;
                                    if (_.isEmpty(n)) return n;
                                    var r = t.toLowerCase();
                                    return _.filter(n, function (t) {
                                        return t.display_name.toLowerCase().indexOf(r) > -1 || t.display_name_pinyin.toLowerCase().indexOf(r) > -1 || t.name.toLowerCase().indexOf(r) > -1
                                    })
                                },
                                sorter: function (t, n, e) {
                                    return n
                                },
                                tpl_eval: function (t, e) {
                                    var r = null;
                                    if ("group" === e.name) r = d.replace(/\$\{([^\}]*)\}/g, function (t, n, r) {
                                        return e[n]
                                    });
                                    else if (e.avatar && "default_avatar.png" != e.avatar && "default.png" != e.avatar) r = t.replace(/\$\{([^\}]*)\}/g, function (t, n, r) {
                                        return t.indexOf("avatar") > 0 ? e[n].replace(".", "_40x40.") : e[n]
                                    });
                                    else {
                                        var s = e.display_name.substring(0, 1).toLocaleUpperCase();
                                        e.firstText = s,
                                            e.shortName = n("avatarShortName")(e.display_name),
                                            e.bgColor = n("avatarBgColor")(e.display_name)["background-color"],
                                            r = l.replace(/\$\{([^\}]*)\}/g, function (t, n, r) {
                                                return e[n]
                                            })
                                    }
                                    return r
                ...
                c.channels && p.channel && i.$watch(c.channels, function (t) {
                    return _.isEmpty(t) ? void o.atwho({
                        at: "#g:",
                        data: []
                    }) : void o.atwho({
                        at: "#g:",
                        ...
                        header_tpl: "<div class='header'>" + a.instant("atwho.GROUP") + "<small>↑&nbsp;↓&nbsp;" + a.instant("atwho.NAVIGATION") + "</small></div>",
                        callbacks: {
                            filter: function (t, n, e) {
                                return _.isEmpty(t) ? n : _.isEmpty(n) ? n : _.filter(n, function (n) {
                                    return n.name_pinyin.toLowerCase().indexOf(t.toLowerCase()) > -1 || n.name.toLowerCase().indexOf(t.toLowerCase()) > -1
                                })
                            },
                            sorter: function (t, n, e) {
                                return n
    ...
    }]).directive("lcCommand", ["lcData", "$translate", "MessageDataContext", function (t, n, e) {
        return {
            require: "^ngModel",
            link: function (r, s, a) {
                r.$watch(a.lcCommand, function (t, e) {
                    var r = _.sortBy(t, function (t) {
                        return t.name
                    });
                    r = _.filter(r, function (t) {
                        return !t.service_id || t.service_id && t.show_in_list
                    });
                    var a = "<li data-value='${name}' class='command'>${name} ${usage_hint} <span class='desc'>" + n.instant("atwho.CUSTOM") + "${desc}</span></li>";
                    s.atwho({
                        at: "/",
                        data: r,
                        show_the_at: !0,
                        ...
                        header_tpl: "<div class='header'>" + n.instant("atwho.COMMAND") + "<small>↑&nbsp;↓&nbsp;" + n.instant("atwho.NAVIGATION") + "</small></div>",
                        callbacks: {
                            matcher: function (t, n) {
                                var e, r;
                                return t = t.replace(/[\-\[\]\/\{\}\(\)\*\+\?\.\\\^\$\|]/g, "\\$&"),
                                    t = "(?:^|\\s)" + t,
                                    r = new RegExp(t + "([A-Za-z0-9_+-.]*)$|" + t + "([^-ÿ]*)$", "gi"),
                                    (e = r.exec(n)) ? e[2] || e[1] : null
                            },
                            filter: function (t, n) {
                                return _.isEmpty(t) ? n : _.isEmpty(n) ? n : _.filter(n, function (n) {
                                    var e = n.usage_hint.toLowerCase();
                                    return n.name.toLowerCase(),
                                    e.indexOf(t.toLowerCase()) > -1 || n.name.toLowerCase().indexOf(t.toLowerCase()) > -1
                                })
                            },
                            tpl_eval: function (t, n) {
                                return (n.desc && !n.type ? a : t).replace(/\$\{([^\}]*)\}/g, function (t, e, r) {
                                    return n[e]
                                })
                            },
                            sorter: function (t, n, e) {
                    ...
                    r.$on("$addCommand", function (n, r) {
                        r.isShow ? t.integration.getCommand(r.service_id).success(function (t) {
                            if (_.find(e.data.commands, function (t) {
                                return t.service_id == r.service_id
                            })) for (var n = 0; n < e.data.commands.length; n++) e.data.commands[n].service_id === t.data.service_id && (e.data.commands[n] = t.data);
                            else e.data.commands.push(t.data)
                        }).error(function (t) {}) : _.remove(e.data.commands, function (t) {
                            return t.service_id == r.service_id
                        })
                    })
            }
    ...
    dashboard.controller("DashboardCtrl", ["$rootScope", "$scope", "$translate", "$window", "toaster", function (t, n, e, r, s) {
        n.user = user,
            t.toaster = s,
            t.resourceServerUrl = "/",
            t.staticResourceServerUrl = "/"
    }]),
    angular.element(document).ready(function () {
        angular.bootstrap(document, ["dashboard"])
    }),
    dashboard.controller("OrganizationCtrl", ["$scope", "$http", "Messager", "$modal", function (t, n, e, r) {
        function s(t) {
            angular.element("#departmentMenu").hide()
        }
        t.tree = {},
            t.status = {},
            t.onSelectionChange = function (n) {
                t.current = n,
                    t.loadUser()
            },
            t.onTreeInit = function () {
                var n = t.tree.getRoot();
                t.tree.select(n.children[0])
            },
            t.onTreeOptionClick = function (n, e) {
                e.stopPropagation();
                var r = angular.element("#departmentMenu");
                r.css({
                    top: e.pageY,
                    left: e.pageX
                }),
            ...
            t.editDepartment = function () {
                ...
                t.open({
                    ...
                    success: function (n) {
                        if (n.oldParent && n.newParent) if (n.oldParent.id !== n.newParent) {
                            var e = t.tree.get(n.oldParent.id);
                            t.tree.reload(e);
                            r = t.tree.get(n.newParent.id);
                            t.tree.reload(r)
                        } else if (n.newParent && !n.oldParent) {
                            var r = t.tree.get(n.newParent.id);
                            t.tree.reload(r)
                        } else(s = t.tree.get(n.department.id)).name = n.department.name;
                        else {
                            var s = t.tree.get(n.department.id);
                            s.name = n.department.name
                        }
                    }
                })
            },
            t.deleteDepartment = function () {
                var r = t.selected,
                    s = t.toaster.wait("正在删除...");
                e.confirm("提示", "确定要删除?").then(function (e) {
                    n.post("/department/delete", {
                        id: r.id
                    }).success(function (n, e, a, i) {
                        n.success ? (s.doSuccess("删除成功"), t.tree.remove(r)) : s.doError("刪除失败：" + n.message)
                    }).error(function (t) {
                        s.doError("刪除失败，稍候再试")
                    })
                })
            ...
            t.loadUser = function () {
                t.status.loading = !0,
                    t.$broadcast("beforeLoadUsers"),
                    t.users = null,
                    n.get("/user/findByDepartment", {
                        params: {
                            departmentId: t.current.id
                        }
                    }).success(function (n, e, r, s) {
                        t.status.loading = !1,
                            n.success ? t.users = n.data : t.status.error = "获取用户列表失败：" + n.message
                    }).error(function (n, e) {
                        t.status.loading = !1,
                            t.status.error = "获取用户列表失败：" + (n.message || e)
                    })
            },
            t.open = function (t) {
                r.open({
                    ...
                    angular.isFunction(t.success) && t.success(n)
                }, function () {})
            }
    }]),
    dashboard.controller("DepartmentCtrl", ["$scope", "$http", "data", "$modalInstance", "Messager", function (t, n, e, r, s) {
        t.showResult = !1,
            t.operation = "添加",
            t.department = {},
            null != e.id ? (n.get("/department/get/" + e.id).success(function (n) {
                t.department = n.data,
                    t.oldParent = t.department.parent
            }), t.operation = "编辑") : t.department = {
                parent: {
                    id: e.parent.id,
                    name: e.parent.name
                }
            },
            t.onSelect = function (n) {
                var e = t.department.parent;
                e && e.id === n.id || (t.department.parent = {
                    id: n.id,
                    name: n.name,
                    code: n.code
                }, t.departmentForm.$setDirty())
            },
            t.save = function () {
                var e = "/department/save";
                t.department.id && (e = "/department/update");
                var s = t.toaster.wait("正在保存...");
                n.post(e, t.department).success(function (n) {
                    n.success ? (s.doSuccess("保存成功"), r.close({
                        department: t.department,
                        oldParent: t.oldParent,
                        newParent: t.department.parent
                    })) : t.result = n
                }).error(function (n, e, r, s, a) {
                    t.result = {
                        success: !1,
                        message: "保存失败:" + e
                    }
            ...
            t.cancel = function () {
                t.departmentForm.$dirty ? s.confirm("提示", "您已经进行了修改，是否要关闭?").then(function (t) {
                    r.dismiss("cancel")
                }, function () {}) : r.dismiss("cancel")
            }
    }]),
    dashboard.controller("IndexCtrl", ["$scope", "$http", "$state", "toaster", function (t, n, e, r) {
        t.report = {
            sumOfSize: "-",
            countOfFile: "-"
        },
            n.get("/report").success(function (n) {
                n.success ? t.report = n.data : r.error("获取报表数据失败")
            }).error(function () {
                r.error("获取报表数据失败")
            }),
            t.getPercent = function (t) {
                return Math.ceil(t.used / t.total * 100)
            },
            t.getType = function (t) {
                var n = t.used / t.total * 100;
                return n < 25 ? "success" : n < 50 ? "info" : n < 75 ? "warning" : "danger"
            },
            t.getColor = function (t) {
                var n = t.used / t.total * 100;
                return n < 25 ? "#27c24c" : n < 50 ? "#23b7e5" : n < 75 ? "#fad733" : "#f05050"
            }
    }]),
    dashboard.controller("LoginLogCtrl", ["$scope", "$http", "$state", function (t, n, e) {
        t.load = function () {
            t.loading = !0,
                t.error = null,
                t.result = null,
                n.get("/login/log/search", {
                    params: t.buildParam()
                }).success(function (n) {
                    t.loading = !1,
                    !1 === n.success && (t.error = n.message),
                        t.result = n.data,
                        t.pagination.pages = Math.ceil(t.result.total / t.pagination.size)
                }).error(function (n, e) {
                    t.loading = !1,
                        t.error = n.message || e
                })
            ...
            t.onPageChange = function () {
                t.pagination.page > t.pagination.pages || t.reload()
            },
            t.condition = {
                start: null,
                end: null,
            ...
            t.buildParam = function () {
                var n = t.condition.start,
                    e = t.condition.end;
                return {
                    size: t.pagination.size,
                    page: t.pagination.page,
                    startDate: n ? moment(n).format("YYYY-MM-DD") + " 00:00:00" : null,
                    endDate: e ? moment(e).format("YYYY-MM-DD") + " 23:59:59" : null,
                    username: t.condition.username,
                    name: t.condition.name
                }
            },
            t.load()
    }]),
    dashboard.controller("ActivityCtrl", ["$scope", "$http", "$state", "$element", function (t, n, e) {
        t.load = function () {
            t.loading = !0,
                t.error = null,
                t.result = null,
                n.get("/activity/search", {
                    params: t.buildParam()
                }).success(function (n) {
                    t.loading = !1,
                    !1 === n.success && (t.error = n.message),
            ...
            t.buildParam = function () {
                ...
                return {
                    ...
                    startDate: n ? moment(n).format("YYYY-MM-DD") + " 00:00:00" : null,
                    endDate: e ? moment(e).format("YYYY-MM-DD") + " 23:59:59" : null,
                    operatorName: t.condition.operator,
                    documentName: t.condition.fileName,
                    operation: t.condition.operation
                }
            },
            t.load()
    }]),
    dashboard.controller("SelectUserCtrl", ["$rootScope", "$scope", "$http", "$modal", "$modalInstance", "options", function (t, n, e, r, s, a) {
        n.loading = !1,
            n.tree = {},
            n.onTreeInit = function () {
                var t = n.tree.getRoot();
                n.tree.select(t)
            },
            ...
            n.selectedIds = [],
            n.select = function (t) {
                n.selectedIds.indexOf(t.id) >= 0 || (!1 === n.multiple && (n.selected = [], n.selectedIds = []), n.selected.push(t), n.selectedIds.push(t.id))
            },
            n.remove = function (t) {
                n.selectedIds.indexOf(t.id) < 0 || (n.selected.splice(n.selected.indexOf(t), 1), n.selectedIds.splice(n.selectedIds.indexOf(t.id), 1))
            },
            n.search = function () {
                n.condition && (n.searching = !0, n.loading = !0, e.get("/user/search", {
                    params: {
                        condition: n.condition
                    }
                }).success(function (t, e, r, s) {
                    n.loading = !1,
                        t.success ? n.result = t.data : n.toaster.pop("error", "提示", t.message)
                }).error(function (t, e) {
                    n.loading = !1,
                        n.toaster.pop("error", "提示", "加载失败：" + e)
                }))
            },
            n.ok = function () {
                s.close(n.selected)
            },
            n.loadUser = function (t) {
                n.loading = !0,
                    e.get("/user/list", {
                        params: {
                            departmentId: t.id
                        ...
                        n.loading = !1,
                            console.log(t),
                            t.success ? n.users = t.data : n.toaster.pop("error", t.message)
                    }).error(function (t, e) {
                        n.loading = !1,
                            n.toaster.pop("error", "加载失败：" + e)
                    })
            },
            n.close = function () {
                s.dismiss("cancel")
    ...
    dashboard.controller("NoticeCtrl", ["$scope", "$http", "Messager", "$rootScope", "$filter", function (t, n, e, r, s) {
        function a() {
            var e = t.toaster.wait("正在加载通知");
            n.get("/notice").success(function (n, r, s, a) {
                n.success ? (e.doClose(), t.notice = n.data) : e.doError("加载通知失败：" + n.message)
            }).error(function (t, n) {
                e.doError("加载通知失败：" + (t.message || n))
            })
        }
        a(),
            t.save = function () {
                t.notice.startDate = s("date")(t.notice.startDate, "yyyy-MM-dd HH:mm:ss"),
                    t.notice.endDate = s("date")(t.notice.endDate, "yyyy-MM-dd HH:mm:ss"),
                    n.post("notice", t.notice).success(function (n, e, s, a) {
                        n.success ? (t.settingForm.$setPristine(), t.settingForm.$setUntouched(), r.toaster.success("保存成功")) : r.toaster.error("保存失败：" + n.message)
                    }).error(function (t, n) {
                        r.toaster.error("保存失败：" + (t.message || n))
                    })
            },
            t.reset = function () {
                t.settingForm.$setPristine(),
                    t.settingForm.$setUntouched(),
                    a()
            }
    }]),
    dashboard.controller("SettingCtrl", ["$scope", "$http", "$state", "$rootScope", function (t, n, e, r) {
        function s() {
            n.get("/setting").success(function (t, n, e, s) {
                t.success ? a(t.data) : r.toaster.pop("error", "提示", "加载安全配置失败：" + data.message)
            }).error(function () {
                r.toaster.pop("error", "提示", "加载安全配置失败：" + status.statusCode)
            })
        }

        function a(n) {
            n.watermarkProperties && Object.keys(t.watermarkProperties).forEach(function (e) {
                n.watermarkProperties.indexOf(e) >= 0 && (t.watermarkProperties[e] = !0)
            }),
                t.setting = n
        }
        t.watermarkProperties = {
            name: !1,
            dept: !1,
            date: !1,
            time: !1,
            ...
            s(),
            t.reset = function () {
                t.securitySettingForm.$setPristine(),
                    t.securitySettingForm.$setUntouched(),
                    s()
            },
            t.save = function () {
                t.setting.watermarkProperties = "",
                    Object.keys(t.watermarkProperties).forEach(function (n) {
                        !0 === t.watermarkProperties[n] && (t.setting.watermarkProperties += n + ";")
                    }),
                    n.post("/setting", t.setting).success(function (n, e, s, a) {
                        n.success ? (t.securitySettingForm.$setPristine(), t.securitySettingForm.$setUntouched(), r.toaster.pop("success", "提示", "保存成功")) : r.toaster.pop("error", "提示", "保存失败：" + n.message)
                    }).error(function () {
                        r.toaster.pop("error", "提示", "保存设置失败：" + status.statusCode)
                    })
            }
    }]),
    dashboard.controller("UserCtrl", ["$scope", "$http", "Messager", "$modal", function (t, n, e, r) {
        t.$on("beforeLoadUsers", function () {
            t.selected = null
        }),
            t.select = function (n) {
                t.selected && t.selected !== n && (t.selected.selected = !1),
                    !0 !== n.selected ? (n.selected = !0, t.selected = n) : (n.selected = !1, t.selected = null)
            },
            t.addUser = function () {
                t.open({
            ...
            t.operation = function (e, r, s) {
                s.stopPropagation(),
                    r.executing = !0;
                var a = "/user/" + e + "/" + r.id,
                    i = t.toaster.wait("正在执行...");
                n.post(a, {}).success(function (t) {
                    r.executing = !1,
                        t.success ? (i.doSuccess("操作成功"), r.status = t.data) : i.doError("操作失败：" + t.message)
                }).error(function (t, n) {
                    r.executing = !1,
                        i.doError("操作失败：" + (t.message || n))
                })
            },
            t.reset = function (r) {
                e.confirm("提示", "确定将该用户的密码重置为系统默认密码?").then(function () {
                    var e = t.selected;
                    e.executing = !0;
                    var r = "/user/reset/" + e.id,
                        s = t.toaster.wait("正在重置密码...");
                    n.post(r, {}).success(function (t) {
                        e.executing = !1,
                            t.success ? s.doSuccess("重置密码成功") : s.doError("重置密码失败：" + t.message)
                    }).error(function (t, n, r, a, i) {
                        e.executing = !1,
                            s.doError("重置密码失败：" + (t.message || n))
                    })
                })
            },
            t.disable = function (n, r) {
                e.confirm("提示", "确定禁用该用户?").then(function () {
                    t.operation("disable", n, r)
                })
            },
            t.enable = function (n, r) {
                e.confirm("提示", "确定启用该用户?").then(function () {
                    t.operation("enable", n, r)
                })
            },
            t.lock = function (n, r) {
                e.confirm("提示", "确定锁定该用户?").then(function () {
                    t.operation("lock", n, r)
                })
            },
            t.unlock = function (n, r) {
                e.confirm("提示", "确定解锁该用户?").then(function () {
                    t.operation("unlock", n, r)
                })
            },
            t.setQuota = function () {
                t.selected;
            ...
            t.transferAdmin = function (r) {
                t.selected.id !== t.user.id ? e.confirm("提示", "管理员权限移交后将会自动退出。确定要移交管理员权限？").then(function () {
                    if (t.selected) {
                        var r = {
                                id: t.selected.id
                            },
                            s = t.toaster.wait("正在移交权限...");
                        t.saving = !0,
                            n.post("/user/admin/transfer", r).success(function (n) {
                                t.saving = !1,
                                    n.success ? (s.doClose(), e.alert("提示", "权限移交成功").then(function () {
                                        window.location.href = "/logout"
                                    })) : s.doError("权限移交失败：" + n.message)
                            }).error(function (n, e) {
                                t.saving = !1,
                                    s.doError("权限移交失败：" + (n.message || e))
                            })
                    }
                }) : e.alert("提示", "不能移交给自己")
            },
    ...
    dashboard.controller("UserFormCtrl", ["$scope", "$http", "data", "$modalInstance", "Messager", function (t, n, e, r, s) {
        t.operation = "添加",
            null != e.id ? (n.get("/user/get/" + e.id).success(function (n) {
                t.user = n.data,
                t.user.department || delete t.user.department
            }), t.operation = "编辑") : t.user = {
                departmentId: e.department.id,
                departmentName: e.department.name
            },
            t.onSelect = function (n) {
                var e = t.user.department;
                e && e.id === n.id || (t.user.departmentId = n.id, t.user.departmentName = n.name, t.userForm.$setDirty())
            },
            t.save = function () {
                t.saving = !0;
                var e = "/user/save";
                t.user.id && (e = "/user/update");
                var s = t.toaster.wait("正在保存...");
                n.post(e, t.user).success(function (n) {
                    t.saving = !1,
                        n.success ? (s.doSuccess("保存成功"), r.close(t.user)) : (s.doError("保存失败：" + n.message), t.result = n)
                }).error(function (n, e, r, a, i) {
                    s.doError("保存失败：" + n.message),
                        t.saving = !1,
                        t.result = {
                            success: !1,
                            message: "保存失败:" + e
            ...
            t.cancel = function () {
                t.userForm.$dirty ? s.confirm("提示", "您已经进行了修改，是否要关闭?").then(function (t) {
                    r.dismiss("cancel")
                }, function () {}) : r.dismiss("cancel")
            }
    }]),
    dashboard.controller("UserQuotaCtrl", ["$scope", "$http", "data", "$modalInstance", "Messager", function (t, n, e, r, s) {
        t.user = e.user,
            t.save = function () {
                t.saving = !0,
                    n.post("/user/set/quota", {
                        id: t.user.id,
                        quota: t.user.quota
                    }).success(function (n) {
                        t.saving = !1,
                            n.success ? r.close() : t.result = n
                    }).error(function (n, e, r, s, a) {
                        t.saving = !1,
                            t.result = {
                                success: !1,
                                message: "保存失败:" + (n.message || e)
                            }
                    })
            },
            t.cancel = function () {
                t.userQutaoForm.$dirty ? s.confirm("提示", "您已经进行了修改，是否要关闭?").then(function (t) {
                    r.dismiss("cancel")
                }, function () {}) : r.dismiss("cancel")
            }
    }]),
    dashboard.controller("UserTransferCtrl", ["$rootScope", "$scope", "$http", "$modal", "$modalInstance", "data", "Messager", function (t, n, e, r, s, a, i) {
        n.loading = !1,
            n.tree = {},
            n.user = a.user,
            n.onTreeInit = function () {
            ...
            n.search = function () {
                n.condition && (n.searching = !0, n.loading = !0, e.get("/user/search", {
                    ...
                    n.loading = !1,
                        t.success ? n.result = t.users : n.toaster.pop("error", "提示", t.message)
                }).error(function (t, e) {
                    n.loading = !1,
                        n.toaster.pop("error", "提示", "加载失败：" + e)
                }))
            },
            n.transfer = function (t) {
                n.select(t),
                !n.selected.id == n.user.id && n.ok()
            },
            n.ok = function () {
                i.confirm("提示", "文档移交后不可恢复，请慎重操作。确定要进行文档？").then(function () {
                    if (n.selected) {
                        var t = {
                                source: n.user.id,
                                target: n.selected.id
                            },
                            r = n.toaster.wait("正在移交文档...");
                        n.saving = !0,
                            e.post("/fs/transfer", t).success(function (t) {
                                n.saving = !1,
                                    t.success ? (r.doSuccess("文档移交成功"), s.close()) : r.doError("文档移交失败：" + t.message)
                            }).error(function (t, e) {
                                n.saving = !1,
                                    r.doError("文档移交失败：" + (t.message || e))
                            })
                    }
                })
            },
            n.loadUser = function (t) {
                n.loading = !0,
                    e.get("/user/list", {
                        ...
                        n.loading = !1,
                            t.success ? n.users = t.users : n.toaster.pop("error", t.message)
                    }).error(function (t, e) {
                        n.loading = !1,
                            n.toaster.pop("error", "加载失败：" + e)
                    })
    ...
    dashboard.controller("UserTransferAdminCtrl", ["$rootScope", "$scope", "$http", "$modal", "$modalInstance", "Messager", function (t, n, e, r, s, a) {
        n.loading = !1,
            n.tree = {},
            n.onTreeInit = function () {
                var t = n.tree.getRoot();
            ...
            n.search = function () {
                n.condition && (n.searching = !0, n.loading = !0, e.get("/user/search", {
                    ...
                    n.loading = !1,
                        t.success ? (a.alert("提示", "您的管理权限已移交").then(function () {
                            window.location.href = "/login"
                        }), n.result = t.users) : n.toaster.pop("error", "提示", t.message)
                }).error(function (t, e) {
                    n.loading = !1,
                        n.toaster.pop("error", "提示", "加载失败：" + e)
                }))
            ...
            n.ok = function () {
                a.confirm("提示", "管理员权限移交后将会自动退出。确定要移交管理员权限？").then(function () {
                    if (n.selected) {
                        var t = {
                                source: n.user.id,
                                target: n.selected.id
                            },
                            r = n.toaster.wait("正在移交权限...");
                        n.saving = !0,
                            e.post("/user/transfer/admin", t).success(function (t) {
                                n.saving = !1,
                                    t.success ? (r.doSuccess("权限移交成功"), s.close()) : r.doError("权限移交失败：" + t.message)
                            }).error(function (t, e) {
                                n.saving = !1,
                                    r.doError("权限移交失败：" + (t.message || e))
                            })
                    }
                })
            },
